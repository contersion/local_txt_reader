...
from app.services.online.detector_runtime_facing_behavior_gate_skeleton import (
    build_detector_runtime_facing_behavior_gate_result,
    evaluate_detector_runtime_facing_behavior_gate_noop,
    prepare_detector_runtime_facing_behavior_gate_input,
)
...
FIXTURE_PATH = Path(__file__).parent / "fixtures" / "online_detector_runtime_facing_behavior_gate_samples.json"
...
def test_runtime_facing_behavior_gate_fixture_file_loads_expected_samples():
    samples = _load_samples()

    assert len(samples) >= 4
    sample_ids = {sample["sample_id"] for sample in samples}
    assert "challenge-behavior-gate-candidate-carried" in sample_ids
    assert "noop-behavior-gate-candidate" in sample_ids


def test_runtime_facing_behavior_gate_input_accepts_runtime_facing_gate_result_and_derived_fields():
    behavior_gate_input = prepare_detector_runtime_facing_behavior_gate_input(
        _build_runtime_facing_gate_result("challenge-gate-candidate-carried")
    )

    assert behavior_gate_input.stage == "search"
    assert behavior_gate_input.recommended_error_code == LegadoRuntimeCode.ANTI_BOT_CHALLENGE
    assert (
        behavior_gate_input.behavior_gate_candidate_kind
        == DetectorRuntimeFacingBehaviorGateCandidateKind.CHALLENGE_CANDIDATE
    )


def test_runtime_facing_behavior_gate_input_rejects_candidate_kind_mismatch():
    runtime_facing_gate_result = _build_runtime_facing_gate_result("challenge-gate-candidate-carried")

    with pytest.raises(ValidationError):
        DetectorRuntimeFacingBehaviorGateInput.model_validate(
            {
                "stage": "search",
                "expected_response_type": "html",
                "runtime_facing_error_gate_result": runtime_facing_gate_result,
                "detector_result": runtime_facing_gate_result.gate_input.detector_result,
                "recommended_error_code": "LEGADO_ANTI_BOT_CHALLENGE",
                "behavior_gate_candidate_kind": "gateway_candidate",
            }
        )


@pytest.mark.parametrize("sample", _load_samples(), ids=lambda sample: sample["sample_id"])
def test_evaluate_detector_runtime_facing_behavior_gate_noop_matches_fixture_expectations(sample):
    runtime_facing_gate_result = _build_runtime_facing_gate_result(sample["runtime_facing_gate_sample_id"])
    result = evaluate_detector_runtime_facing_behavior_gate_noop(runtime_facing_gate_result)

    assert result.outcome.value == sample["expected_behavior_gate_outcome"]
    assert result.behavior_gate_decision == DetectorRuntimeFacingBehaviorGateDecision.BEHAVIOR_GATE_DECISION_NOOP
    assert (
        result.behavior_gate_input.behavior_gate_candidate_kind.value
        == sample["expected_behavior_gate_candidate_kind"]
    )
    if sample["expected_recommended_error_code"] is None:
        assert result.behavior_gate_candidate is None
    else:
        assert result.behavior_gate_candidate is not None
        assert result.behavior_gate_candidate.recommended_error_code is not None
        assert (
            result.behavior_gate_candidate.recommended_error_code.value
            == sample["expected_recommended_error_code"]
        )


def test_build_runtime_facing_behavior_gate_result_requires_candidate_for_candidate_carried():
    behavior_gate_input = prepare_detector_runtime_facing_behavior_gate_input(
        _build_runtime_facing_gate_result("challenge-gate-candidate-carried")
    )

    with pytest.raises(ValidationError):
        DetectorRuntimeFacingBehaviorGateResult.model_validate(
            {
                "outcome": DetectorRuntimeFacingBehaviorGateOutcome.BEHAVIOR_GATE_CANDIDATE_CARRIED,
                "behavior_gate_decision": DetectorRuntimeFacingBehaviorGateDecision.BEHAVIOR_GATE_DECISION_NOOP,
                "behavior_gate_input": behavior_gate_input,
            }
        )


def test_build_runtime_facing_behavior_gate_result_returns_no_candidate_for_no_gate_candidate():
    behavior_gate_input = prepare_detector_runtime_facing_behavior_gate_input(
        _build_runtime_facing_gate_result("no-gate-candidate")
    )
    result = build_detector_runtime_facing_behavior_gate_result(behavior_gate_input)

    assert result.outcome == DetectorRuntimeFacingBehaviorGateOutcome.NO_BEHAVIOR_GATE_CANDIDATE
    assert result.behavior_gate_decision == DetectorRuntimeFacingBehaviorGateDecision.BEHAVIOR_GATE_DECISION_NOOP
    assert result.behavior_gate_candidate is None


def test_execute_stage_success_runs_runtime_facing_behavior_gate_helper_without_changing_response(monkeypatch):
    ...
    def fake_evaluate_behavior_gate(runtime_facing_gate_result):
        observed["runtime_facing_gate_result"] = runtime_facing_gate_result
        return {"behavior_gate": "noop"}

    monkeypatch.setattr(
        source_engine,
        "evaluate_detector_runtime_facing_behavior_gate_noop",
        fake_evaluate_behavior_gate,
        raising=False,
    )
    ...
    assert observed["runtime_facing_gate_result"].outcome.value == "gate_candidate_carried"


def test_execute_stage_error_runs_runtime_facing_behavior_gate_helper_without_changing_error(monkeypatch):
    request_profile = _build_request_profile()
...
def test_execute_stage_success_ignores_runtime_facing_behavior_gate_helper_failure(monkeypatch):
    request_profile = _build_request_profile()
    raw_response = RawFetchResponse(
        requested_url=request_profile.url,
        final_url=request_profile.url,
    ...
    monkeypatch.setattr(
        source_engine,
        "evaluate_detector_runtime_facing_behavior_gate_noop",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(RuntimeError("internal behavior gate failure")),
        raising=False,
    )
...
def test_execute_stage_error_ignores_runtime_facing_behavior_gate_helper_failure(monkeypatch):
    request_profile = _build_request_profile()
    fetch_error = FetchServiceError(
        "Request timeout while fetching https://example.com/search?q=test",
        code=LegadoRuntimeCode.REQUEST_TIMEOUT,
...
def test_fetch_service_source_does_not_import_runtime_facing_behavior_gate_skeleton():
    source = inspect.getsource(fetch_service)

    assert "detector_runtime_facing_behavior_gate_skeleton" not in source


def test_response_guard_source_does_not_import_runtime_facing_behavior_gate_skeleton():
    source = inspect.getsource(response_guard_service)

    assert "detector_runtime_facing_behavior_gate_skeleton" not in source


def test_content_parse_source_does_not_import_runtime_facing_behavior_gate_skeleton():
