...
from app.services.online.detector_runtime_facing_gate_skeleton import (
    build_detector_runtime_facing_gate_result,
    evaluate_detector_runtime_facing_gate_noop,
    prepare_detector_runtime_facing_gate_input,
)
...
FIXTURE_PATH = Path(__file__).parent / "fixtures" / "online_detector_runtime_facing_gate_samples.json"
...
def test_runtime_facing_gate_fixture_file_loads_expected_samples():
    samples = _load_samples()

    assert len(samples) >= 4
    sample_ids = {sample["sample_id"] for sample in samples}
    assert "challenge-gate-candidate-carried" in sample_ids
    assert "noop-gate-candidate" in sample_ids


def test_runtime_facing_gate_input_accepts_mapping_result_and_derived_fields():
    gate_input = prepare_detector_runtime_facing_gate_input(
        _build_runtime_error_mapping_result("challenge-mapping-candidate-carried")
    )

    assert gate_input.stage == "search"
    assert gate_input.recommended_error_code == LegadoRuntimeCode.ANTI_BOT_CHALLENGE
    assert gate_input.gate_candidate_kind == DetectorRuntimeFacingGateCandidateKind.CHALLENGE_CANDIDATE


def test_runtime_facing_gate_input_rejects_gate_candidate_kind_mismatch():
    runtime_error_mapping_result = _build_runtime_error_mapping_result("challenge-mapping-candidate-carried")

    with pytest.raises(ValidationError):
        DetectorRuntimeFacingGateInput.model_validate(
            {
                "stage": "search",
                "expected_response_type": "html",
                "runtime_error_mapping_result": runtime_error_mapping_result,
                "detector_result": runtime_error_mapping_result.mapping_input.detector_result,
                "recommended_error_code": "LEGADO_ANTI_BOT_CHALLENGE",
                "gate_candidate_kind": "gateway_candidate",
            }
        )


@pytest.mark.parametrize("sample", _load_samples(), ids=lambda sample: sample["sample_id"])
def test_evaluate_detector_runtime_facing_gate_noop_matches_fixture_expectations(sample):
    runtime_error_mapping_result = _build_runtime_error_mapping_result(sample["runtime_error_mapping_sample_id"])
    result = evaluate_detector_runtime_facing_gate_noop(runtime_error_mapping_result)

    assert result.outcome.value == sample["expected_gate_outcome"]
    assert result.gate_decision == DetectorRuntimeFacingGateDecision.GATE_DECISION_NOOP
    assert result.gate_input.gate_candidate_kind.value == sample["expected_gate_candidate_kind"]
    if sample["expected_recommended_error_code"] is None:
        assert result.gate_candidate is None
    else:
        assert result.gate_candidate is not None
        assert result.gate_candidate.recommended_error_code is not None
        assert result.gate_candidate.recommended_error_code.value == sample["expected_recommended_error_code"]


def test_build_runtime_facing_gate_result_requires_candidate_for_gate_candidate_carried():
    gate_input = prepare_detector_runtime_facing_gate_input(
        _build_runtime_error_mapping_result("challenge-mapping-candidate-carried")
    )

    with pytest.raises(ValidationError):
        DetectorRuntimeFacingGateResult.model_validate(
            {
                "outcome": DetectorRuntimeFacingGateOutcome.GATE_CANDIDATE_CARRIED,
...
def test_build_runtime_facing_gate_result_returns_no_candidate_for_no_mapping_candidate():
    gate_input = prepare_detector_runtime_facing_gate_input(
        _build_runtime_error_mapping_result("no-visible-mapping-candidate")
    )
    result = build_detector_runtime_facing_gate_result(gate_input)

    assert result.outcome == DetectorRuntimeFacingGateOutcome.NO_GATE_CANDIDATE
    assert result.gate_decision == DetectorRuntimeFacingGateDecision.GATE_DECISION_NOOP
    assert result.gate_candidate is None


def test_execute_stage_success_runs_runtime_facing_gate_helper_without_changing_response(monkeypatch):
    ...
    def fake_evaluate_gate(runtime_error_mapping_result):
        observed["runtime_error_mapping_result"] = runtime_error_mapping_result
        return {"gate": "noop"}
    ...
    assert observed["runtime_error_mapping_result"].outcome.value == "mapping_candidate_carried"


def test_execute_stage_error_runs_runtime_facing_gate_helper_without_changing_error(monkeypatch):
    request_profile = _build_request_profile()
...
def test_execute_stage_success_ignores_runtime_facing_gate_helper_failure(monkeypatch):
    request_profile = _build_request_profile()
    raw_response = RawFetchResponse(
        requested_url=request_profile.url,
        final_url=request_profile.url,
...
def test_execute_stage_error_ignores_runtime_facing_gate_helper_failure(monkeypatch):
    request_profile = _build_request_profile()
    fetch_error = FetchServiceError(
        "Request timeout while fetching https://example.com/search?q=test",
        code=LegadoRuntimeCode.REQUEST_TIMEOUT,
...
def test_fetch_service_source_does_not_import_runtime_facing_gate_skeleton():
    source = inspect.getsource(fetch_service)

    assert "detector_runtime_facing_gate_skeleton" not in source


def test_response_guard_source_does_not_import_runtime_facing_gate_skeleton():
    source = inspect.getsource(response_guard_service)

    assert "detector_runtime_facing_gate_skeleton" not in source


def test_content_parse_source_does_not_import_runtime_facing_gate_skeleton():
